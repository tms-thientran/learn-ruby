...
class Deleter
  ...
  def delete_file
    ...
    total_size = file_mark_deletes.sum { |f| f[:file_size].to_f }.to_unit
  ...
  def safe_delete_file(file, root_path = 'safe_delete')
    ...
    begin
      folder_path = file_path.sub(%r{^/}, '') # bỏ dấu / đầu tiên
  ...
  def print_summary
    ...
    puts "💾 Tổng dung lượng đã giải phóng: #{total_size.to_unit}"
    puts '=' * 50

    # Hiển thị thông báo macOS (nếu có)
    send_notification(total_size) if total_size.positive?
  end

  def send_notification(size_b)
    return unless RUBY_PLATFORM.include?('darwin') # Chỉ chạy trên macOS

    message = "Hoàn tất dọn dẹp #{size_b.to_unit} - #{@deleted_files.size} files"
