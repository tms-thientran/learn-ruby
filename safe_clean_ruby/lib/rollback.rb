...
class Rollback
  ...
  def rollback_all_files
    ...
    log_files.each do |log_file|
      data_csvs = get_data_from_file_path(log_file)
      data_uniq = data_csvs.uniq { |data_csv| data_csv[:original_path] }

      mv_rollback_folder(data_uniq)
    end
  end
  ...
  def get_data_from_file_path(last_file)
    ...
    puts "Tên file rollback #{File.basename(last_file)}"
  ...
  def handle_rollback_file(file, _folder_rollback)
    deleted_path = file[:deleted_file_path]
  ...
  def print_summary(elapsed_time)
    ...
    return unless @failed_files.any?
